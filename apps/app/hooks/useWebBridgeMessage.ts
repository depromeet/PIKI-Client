  ...
  const onMessage = useCallback(
    (event: WebViewMessageEvent) => {
      try {
        ...
        if (!message) return console.log('[WEBVIEW] 수신 메시지 존재하지 않음');

        const parsedMessage = JSON.parse(message);
        if (!isWebBridgeMessageT(parsedMessage))
          return console.log('[WEBVIEW] 수신 메시지 형식 오류', parsedMessage);

        console.log('[WEBVIEW] 수신 메시지:', parsedMessage);
        handler(parsedMessage);
      } catch {
        console.log('[WEBVIEW] 오류', event.nativeEvent.data);
      }
    },
    [handler]
  );
