...
function Input({ label, helperText, status = 'default', left, right, className, ...props }: InputProps) {
  return (
    <div className="flex flex-col gap-1.5">
      {label && <label className="body-2-semibold text-gray-950">{label}</label>}
      <div
      ...
      {helperText && (
        <p className={`body-2-regular ${status === 'error' ? 'text-red-600' : 'text-gray-300'}`}>
          {helperText}
        </p>
      )}
    </div>
