...
/** Vercel shallow clone에 web-v* 태그를 fetch한다. */
const fetchWebTagsOnVercel = () => {
  try {
    execSync('git fetch origin --tags --depth=1', {
      encoding: 'utf-8',
      stdio: ['pipe', 'pipe', 'ignore'],
    });
    console.log('[WEB VERSION] git fetch --tags 완료');
  } catch {
    console.log('[WEB VERSION] git fetch --tags 실패');
  }
};

/** 로컬 web-v* 태그 목록에서 최신 버전 추출 */
const getWebVersionFromTagList = () => {
  try {
...
const getWebVersion = () => {
  if (process.env.VERCEL) {
    ...
    fetchWebTagsOnVercel();

    const fromTagList = getWebVersionFromTagList();
